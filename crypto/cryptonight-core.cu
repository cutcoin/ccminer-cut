...
static __forceinline__ __device__ uint4 rotate4x32(const uint4 &a, const uint32_t cnt) {
  return make_uint4(ROTL32(a.x, cnt), ROTL32(a.y, cnt), ROTL32(a.z, cnt), ROTL32(a.w, cnt));
}
...
static __forceinline__ __device__ uint4 &operator += (uint4 &a, const uint4 &b) {
  a.x += b.x;
  a.y += b.y;
  a.z += b.z;
  a.w += b.w;
...
//__launch_bounds__(128,12) /* force 40 regs to allow -l ...x32 */
#endif
void cut_gpu_phase2(const uint32_t threads, const uint16_t bfactor, const uint32_t partidx,
  uint64_t * __restrict__ d_long_state, uint32_t * __restrict__ d_ctx_a, uint32_t * __restrict__ d_ctx_b)
{
  __shared__ __align__(16) uint32_t sharedMemory[1024];
  cn_aes_gpu_init(sharedMemory);
  ...
  if (thread < threads)
  {
    const uint32_t batchsize = ITER >> (1 + bfactor);
    const uint32_t start = partidx * batchsize;
    const uint32_t end = start + batchsize;
    ...
    {
      uint4 C, tmp;
      uint32_t j = (A.x & 0x1FFFC0) >> 3;
      tmp = AS_UINT4(&long_state[j]);
      tmp = rotate4x32(tmp, B.x & 31);
      tmp += AS_UINT4(&long_state[j + 2]);
      tmp = rotate4x32(tmp, B.y & 31);
      tmp += AS_UINT4(&long_state[j + 4]);
      tmp = rotate4x32(tmp, B.z & 31);
      tmp += AS_UINT4(&long_state[j + 6]);
      tmp = rotate4x32(tmp, B.w & 31);

      cn_aes_single_round_c((uint8_t*)sharedMemory, tmp, A, &C);

      AS_UINT4(&long_state[j]) ^= rotate4x32(B, A.x & 31) + C;
      AS_UINT4(&long_state[j + 2]) ^= rotate4x32(B, A.y & 31) + C;
      AS_UINT4(&long_state[j + 4]) ^= rotate4x32(B, A.z & 31) + C;
      AS_UINT4(&long_state[j + 6]) ^= rotate4x32(B, A.w & 31) + C;

      j = (C.x & 0x1FFFC0) >> 3;

      tmp = AS_UINT4(&long_state[j]);
      tmp = rotate4x32(tmp, A.x & 31);
      tmp -= AS_UINT4(&long_state[j + 2]);
      tmp = rotate4x32(tmp, A.y & 31);
      tmp -= AS_UINT4(&long_state[j + 4]);
      tmp = rotate4x32(tmp, A.z & 31);
      tmp -= AS_UINT4(&long_state[j + 6]);
      tmp = rotate4x32(tmp, A.w & 31);

      A += C * tmp;

      AS_UINT4(&long_state[j]) ^= rotate4x32(A, tmp.x & 31);
      AS_UINT4(&long_state[j + 2]) ^= rotate4x32(A, tmp.y & 31);
      AS_UINT4(&long_state[j + 4]) ^= rotate4x32(A, tmp.z & 31);
      AS_UINT4(&long_state[j + 6]) ^= rotate4x32(A, tmp.w & 31);
	...
	uint32_t *d_ctx_a, uint32_t *d_ctx_b, uint32_t *d_ctx_key1, uint32_t *d_ctx_key2, int variant, uint64_t *d_ctx_tweak)
{
    ...
    else if (cryptonight_fork == 2)
			cut_gpu_phase2 <<<grid, b>>> (throughput, bfactor, i, d_long_state, d_ctx_a, d_ctx_b);
