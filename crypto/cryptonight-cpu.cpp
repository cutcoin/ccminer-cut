...
static void copy_block(void* dst, const void* src) {
  copy_block((uint8_t *)dst, (const uint8_t *)src);
}
...
static void cryptonight_hash_ctx(void* output, const void* input, const size_t len, struct cryptonight_ctx* ctx, const int variant)
{
  ...
  if (cryptonight_fork == 2 ) {
    uint32_t a[4], b[4];
    copy_block(a, ctx->a);
    copy_block(b, ctx->b);
    for (i = 0; i < ITER / 2; i++) {
      uint32_t c[4], tmp[4];
      copy_block(tmp, ctx->long_state + (a[0] & 0x1FFFC0));
      rotate4x32(tmp, b[0] & 31);
      add4x32(tmp, ctx->long_state + (a[0] & 0x1FFFC0) + AES_BLOCK_SIZE);
      rotate4x32(tmp, b[1] & 31);
      add4x32(tmp, ctx->long_state + (a[0] & 0x1FFFC0) + AES_BLOCK_SIZE * 2);
      rotate4x32(tmp, b[2] & 31);
      add4x32(tmp, ctx->long_state + (a[0] & 0x1FFFC0) + AES_BLOCK_SIZE * 3);
      rotate4x32(tmp, b[3] & 31);

      aesb_single_round((uint8_t *)tmp, (uint8_t *)c, (uint8_t *)a);

      for(j = 0; j < 4; j++) {
        rotate4x32_3(tmp, b, a[j] & 31);
        add4x32(tmp, c);
        xor4x32(ctx->long_state + (a[0] & 0x1FFFC0) + j * AES_BLOCK_SIZE, tmp);
      }

      copy_block(tmp, ctx->long_state + (c[0] & 0x1FFFC0));
      rotate4x32(tmp, a[0] & 31);
      sub4x32(tmp, ctx->long_state + (c[0] & 0x1FFFC0) + AES_BLOCK_SIZE);
      rotate4x32(tmp, a[1] & 31);
      sub4x32(tmp, ctx->long_state + (c[0] & 0x1FFFC0) + AES_BLOCK_SIZE * 2);
      rotate4x32(tmp, a[2] & 31);
      sub4x32(tmp, ctx->long_state + (c[0] & 0x1FFFC0) + AES_BLOCK_SIZE * 3);
      ...
      for(j = 0; j < 4; j ++) {
        ...
        xor4x32(ctx->long_state + (b[0] & 0x1FFFC0) + j * AES_BLOCK_SIZE, c);
      }
...
void cut_hash(void* output, const void* input)
{
  cryptonight_fork = 2;
  cryptonight_hash_variant(output, input, 76, 7);
}
